...
function listarTablaMultiplicar(pedido,respuesta) {
	/*
	Utilizamos el objeto url llamando al método parse y pasando dos parámetros: pedido.url y el valor true. Mediante el objeto que nos devuelve accedemos a la propiedad query y en este objeto tenemos una propiedad llamada igual a nuestro parámetro 'numero'. Si tiene más parámetros los accedemos de la misma forma.
	*/
	var valor=url.parse(pedido.url,true).query.numero;
    respuesta.writeHead(200, {'Content-Type': 'text/html'});
	var pagina='<!doctype html><html><head></head><body>';
	for(var f=1;f<=10;f++) {
